...
# save the factors
SAVE_PATH = r"E:\My_Strategies_CTA"   
...
def save(data, path):
    # wb表示以二进制写方式打开，只能写文件，如果文件不存在，创建该文件；如果文件已存在，则覆盖写
...
# 自己手写了一个moving_average，相当于设置了min_periods=1，然后rolling后取mean
# 通过cumsum的作差来计算rolling sum，然后除以period长度的得到mean
...
# 指数加权移动平均，手写了一个，相当于ewm后取mean
# 不过写的这个只能接受一个参数，就是半衰期，多久后数值变为一半，不能接受com或者alpha或者span
# alpha = 1/(1+com) = 2/(span+1) = 1-exp(log(0.5)/halflife)
def ewma(x, halflife, init=0, adjust=False):
    # series的append操作，相当于在传入的Series前面加上一个0
...
def get_signal_pnl(file, product, signal_name, thre_mat, reverse=1, tranct=1.1e-4, max_spread=0.61, tranct_ratio=True, 
                   HEAD_PATH=r"E:\My_Strategies_CTA", SIGNAL_PATH=r"E:\My_Strategies_CTA", atr_filter=0, rebate=0):
    ## load data
...
def create_signal_path(signal_list, product, HEAD_PATH):
    ...
    for cartesian in itertools.product(*signal_list.params.values()):
        ...
        os.makedirs(HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name, exist_ok=True)
        print(HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name)

def get_signal_mat(signal_mat, signal_name, product, file_name, HEAD_PATH=r"E:\My_Strategies_CTA"):
    S = load(HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name+"\\"+file_name)
    S[np.isnan(S)] = 0
...
def get_signal_stat(signal_name, thre_mat, product, all_dates, CORE_NUM, split_str="2018", reverse=1, tranct=1.1e-4, 
                    max_spread=0.61, tranct_ratio=True, min_pnl=2, min_num=20, atr_filter=0, HEAD_PATH=r"E:\My_Strategies_CTA", SIGNAL_PATH=r"E:\My_Strategies_CTA"):
    train_sample = all_dates<split_str
...
def get_all_signal(file_list, product, signal_name, period, HEAD_PATH=r"E:\My_Strategies_CTA", SIGNAL_PATH=r"E:\My_Strategies_CTA"):
...
def par_get_all_signal(signal_name, file_list, product, period, SAVE_PATH=r"E:\My_Strategies_CTA"):
    n_files = len(file_list)
    all_signal = np.array([])
    for file in file_list:
        S = load(SAVE_PATH+"\\factor_pkl\\"+product+"\\"+signal_name+"\\"+file) ## signal
        good = load(SAVE_PATH+"\\good_pkl\\"+product+"\\"+file) ## good signal
        signal = S[good]
        moving_average(signal,period)
        chosen = (np.arange(len(signal))+1) % period==0
        all_signal = np.concatenate((all_signal, signal[chosen]), axis=0)
    save(all_signal, SAVE_PATH+"\\all_signals\\"+product+"."+signal_name+".pkl")

def par_get_arb_all_signal(signal_name, file_list, product_x, product_y, period, HEAD_PATH=r"E:\My_Strategies_CTA"):
    n_files = len(file_list)
    all_signal = np.array([])
    for file in file_list:
        S_x = load(HEAD_PATH+"\\factor_pkl\\"+product_x+"\\"+signal_name+"\\"+file)
        S_y = load(HEAD_PATH+"\\factor_pkl\\"+product_y+"\\"+signal_name+"\\"+file)
...
def add_min_max(file, period_list):
    ...
    for period in period_list:
        ...
        data.iloc[:period, ("min."+str(period))] = np.minimum.accumulate(data["wpr"].iloc[:period])
...
def get_signal_pnl_close(file, product, signal_name, thre_mat, reverse=1, rebate=0, HEAD_PATH=r"E:\My_Strategies_CTA"):
    ## load data
    data = load(HEAD_PATH+"\\night_pkl_tick\\"+product+"\\"+file)
    ## load signal
    S = load(HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name+"\\"+file)
...
def evaluate_signal(signal, all_dates, product, min_pnl, min_num, 
                    CORE_NUM, HEAD_PATH=r"E:\My_Strategies_CTA", SIGNAL_PATH=r"E:\My_Strategies_CTA", period=4096, split_str="2018", tranct=1.1e-4, 
                    max_spread=0.61, tranct_ratio=True, atr_filter=0, save_path="signal_result_atr",reverse=0):
...
def par_get_daily_pred(file_name, product, coef, strat, HEAD_PATH, SAVE_PATH):
    ...
    save(S, SAVE_PATH+"\\factor_pkl\\"+product+"\\"+strat+"\\"+file_name)      

    
def get_daily_gbm(file_name, product, signal_list, model, strat, HEAD_PATH, SAVE_PATH, train_std, thre=float('Inf')):
    signal_mat = load(SAVE_PATH+"\\signal_mat_pkl\\"+product+"\\"+file_name).T/train_std
...
def get_multiple_lasso_roll_model(train_start, train_end, y_signal, n_signal, daily_ticks, all_mat, forward_len, strat, 
                          single_product, combine_product, period=4096, SAVE_PATH=r"E:\My_Strategies_CTA"):
...
def get_glmnet_ensemble_roll_model(train_start, train_end, forward_len, alpha=1, start_year=2018, period=2048):
    ...
    os.makedirs(HEAD_PATH+"\\roll_model", exist_ok=True)
    os.makedirs(HEAD_PATH+"\\roll_model\\"+product, exist_ok=True)
    save(model_coef, HEAD_PATH+"\\roll_model\\"+product+"\\"+strat+"."+str(train_start)+"."+str(train_end)+".pkl")
