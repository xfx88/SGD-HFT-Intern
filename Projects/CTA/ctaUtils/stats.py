...
class FactorData(pd.DataFrame):
    ...
    def __getitem__(self, key):
        try:
            ...
            s = load(self._fHEAD_PATH+"\\factor_pkl\\"+self._fproduct+"\\"+key+"\\"+self._fdate)
            self[key] = s
        return s
        
        
# inspect包可以获取关于一个object/class/function的详细信息
import inspect
from collections import OrderedDict

# 因子模版，作为superclass，同时是object类的subclass
class factor_template(object):
    factor_name = ""
    
    # 因子参数，一般取[1024, 2048, 4096]
    params = OrderedDict([
    ...
    def form_info(self):
        # inspect.getsource(object)函数以字符串的形式返回某个py对象的源代码
        # 这里就是返回因子formula的源代码
    ...
    # 魔法方法，__repr__和__str__作用都是返回一个string representation of the class
    # 区别是repr更正式，str可读性更高，repr可以清晰描述对象的信息类型值等等
    # 这里人为定义就相同处理了
    def __repr__(self):
        return self.info()
...
def build_simple_signal(file_name, signal_list, product, HEAD_PATH):
    ...
    for cartesian in itertools.product(*signal_list.params.values()):
        ...
        path = HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name+"\\"+file_name[-12:]
...
def build_composite_signal(file_name, signal_list, product, HEAD_PATH, n=12):
    ...
    for cartesian in itertools.product(*signal_list.params.values()):
        ...
        for i in range(len(cartesian)):
            signal_name = signal_name.replace(keys[i], str(cartesian[i])) 
        path = HEAD_PATH+"\\factor_pkl\\"+product+"\\"+signal_name+"\\"+file_name[-n:]
        S = signal_list.formula(data, *cartesian)
        save(S, path)
