...
class MultiHeadAttentionWindow(MultiHeadAttention):
    ...
    def forward(self,
        ...
        # query = F.pad(query.transpose(1, 2), (self._padding, self._padding), 'constant').transpose(1, 2) # shape: [batch_size, (pad, len_data, pad), dim_q]
        ...
        # attention = ffold(attention, self._window_size, self._step)
...
def ffold(x, window, step):
    x = x[:, :-1, step:, :]
    elements = []
    elements.append(x[:, :, :step, :])
    for i in range(int(x.size(2) / step) - 1):
        elements.append(x[:, -1:, step * (i + 1): step * (i + 2), :])
