...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
                ...
                key: torch.Tensor,
                value: torch.Tensor,
                mask: Optional[str] = 'subsequent') -> torch.Tensor:
        ...
        # Scaled Dot Product
        _scores = torch.bmm(queries, keys.transpose(1, 2)) / np.sqrt(K)

        # Compute local map mask
        if self._attention_size is not None:
            attention_mask = generate_local_map_mask(K, self._attention_size, mask_future=False, device=_scores.device)
            _scores = _scores.masked_fill(attention_mask, float('-inf'))

        # Compute future mask
        if mask == "subsequent":
            future_mask = torch.triu(torch.ones((K, K)), diagonal=1).bool()
            future_mask = future_mask.to(_scores.device)
            _scores = _scores.masked_fill(future_mask, float('-inf'))

        # Apply sotfmax
        _scores = F.softmax(_scores, dim=-1)

        attention = torch.bmm(_scores, values)
...
class MultiHeadAttentionWindow(MultiHeadAttention):
    ...
    def __init__(self,
        ...
        self._step = self._window_size - self._padding * 2
        # self._step = padding
    ...
    def forward(self,
        ...
        query = F.pad(query.transpose(1, 2), (self._padding, self._padding), 'constant').transpose(1, 2)
        key = F.pad(key.transpose(1, 2), (self._padding, self._padding), 'constant').transpose(1, 2)
        value = F.pad(value.transpose(1, 2), (self._padding, self._padding), 'constant').transpose(1, 2)
