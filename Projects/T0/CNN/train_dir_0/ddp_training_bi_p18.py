...
class MatrixCrossEntropy(nn.Module):
    def __init__(self, label_nums = 3, local_rank = 0):
        ...
        self.weight = F.normalize(1 / torch.Tensor([[20, 1, 1],[15, 1, 1],[10, 1, 1]]), dim = 1, p = 1).to(local_rank)
    ...
    def forward(self, pred, target):
        ...
        loss = (temp * self.weight * pred).sum()
...
class ConvLstmNet(nn.Module):
    def __init__(self, in_features, seq_len, out_features):
        ...
        self.seqExtractor = nn.Linear(seq_len, 3)

        self.conv_3 = CNNBlock(in_channels=256,out_channels=512,kernel_size=(3,),padding=(1,))

        self.labelExtractor =  nn.Conv1d(in_channels=512, out_channels=out_features, kernel_size=(1,))
    ...
    def forward(self, x):
        ...
        x = self.labelExtractor(x)
        # x = self.dropout(x)
        x = self.conv_3(x)



        x = self.seqExtractor(x).permute(0,2,1)
