...
def submit_train_data_date(month, ticker, values, db):
    ...
    for date in values:
        ...
        df = df[col_factors].fillna(0)

        if len(df) > 4800: continue

        ut.save_data_to_redis(rs, bytes(f'df_{ticker}_{date}_origin', encoding='utf-8'), df)
...
def parallel_submit_ticker_monthly_numpy_train(db):
    ...
    for month, ticker_dates in ticker_date_dict.items():
        Parallel(n_jobs=48, verbose=10, timeout=10000)(delayed(submit_train_data_date)(month, ticker, dates, db)
