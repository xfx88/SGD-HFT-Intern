...
class MatrixCrossEntropy(nn.Module):
    def __init__(self, class_weight, label_nums = 4, local_rank = 0, gamma = 2):
        ...
        self.label_weight = torch.Tensor([3/10, 3/10, 3/10, 1/10]).to(local_rank)
        self.base_loss = [nn.CrossEntropyLoss(weight=self.class_weight[i], reduction="sum") for i in range(label_nums)]
...
def shard_keys(start_date, end_date, seq_len = 50, time_step = 1, db = DB_ID):
    ...
    keys_to_shard = [x.decode(encoding = 'utf-8') for x in all_redis_keys
                    ...
                    and (x.decode(encoding = 'utf-8').split('_')[1] <= end_date[4:6])
                    and (x.decode(encoding = 'utf-8').split('_')[1] >= start_date[4:6])
...
def train(local_rank, world_size, world_dict, shard_dict, validation = False, Resume = None):
    ...
    if Resume:
        model_name = "CNNLstmCLS_epoch_79_bs10000_sl64_ts2.pth.tar"
    ...
    net = ConvLstmNet(in_features=INPUT_SIZE,
                      seq_len=SEQ_LEN,
                      out_features=OUTPUT_SIZE // 4).to(local_rank)
    ...
    optimizer = torch.optim.Adam(ddp_net.parameters(), lr = 0.5e-4)
    ...
    for epoch_idx in range(prior_epochs, EPOCHS):
        ...
        for batch_idx, (x, y) in enumerate(train_dataset):
            ...
            if (batch_idx + 1) % 50 == 0 or batch_idx == 0 or batch_idx == datalen - 1:
        ...
        if local_rank == 0:
            ...
            if validation:
                ...
                p2_accuracy_val, p5_accuracy_val, p18_accuracy_val, diff_accuracy_val = \
                    val_result['p2']['accuracy'], val_result['p5']['accuracy'], val_result['p18']['accuracy'], val_result['diff']['accuracy']
...
def validate(net, validation_dataset, lossfn, local_rank):
    ...
    summary_p2 = MetricRUC(name = 'p2', classnum=OUTPUT_SIZE//4)
    summary_p5 = MetricRUC(name = 'p5', classnum=OUTPUT_SIZE//4)
    summary_p18 = MetricRUC(name = 'p18', classnum=OUTPUT_SIZE//4)
...
def main_train(LOGGER):
    ...
    # world_size = 1
    mp.spawn(train,
             args=(world_size, world_dict, shard_dict, True, True),
             nprocs=world_size,
             join=True)
