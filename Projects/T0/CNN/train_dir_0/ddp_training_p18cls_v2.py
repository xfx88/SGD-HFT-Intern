...
class FocalLoss(nn.Module):
    ...
    def __init__(self, class_num, alpha = torch.tensor([1, 5, 5]), gamma=2, size_average=True):
        super(FocalLoss, self).__init__()
        if alpha is None:
            self.alpha = 0.25 * Variable(torch.ones(class_num, 1))
        else:
...
class ConvLstmNet(nn.Module):
    def __init__(self, in_features, seq_len, out_features):
        ...
        self.conv_1 =  CNNBlock(in_channels= in_features,
                                out_channels = 64,
                                kernel_size=(3, ),
                                dilation=(4,),
                                padding=(4, )) # seq: 64 -> 32

        self.conv_2 = CNNBlock(in_channels=64,
                               out_channels=128,
                               kernel_size=(3,),
                               padding=(1,)) # seq: 32 -> 16

        self.conv_3 = CNNBlock(in_channels=128,
                               out_channels=256,
                               kernel_size=(5,),
                               dilation=(3,),
                               padding=(6,)) # seq: 16 -> 8

        self.conv_4 = CNNBlock(in_channels=256,
                               out_channels=512,
                               kernel_size=(3,),
                               padding=(1,),
                               max_pool_kernel=(8,)) # seq: 8 -> 4

        # self.conv_5 = CNNBlock(in_channels=256,
        #                        out_channels=512,
        #                        kernel_size=(5,),
        #                        padding=(2,),
        #                        max_pool_kernel=(4,)) # seq: 4 -> 1

        # self.downsample1 = nn.Linear(256, 3)
        # self.downsample2 = nn.Linear(256, 3)
        self.downsample3 = nn.Linear(512, 3)
    ...
    def forward(self, x):
        ...
        # y_p2 = self.downsample1(conv_feat)
        # y_p5 = self.downsample2(conv_feat)
        y_p18 = self.downsample3(conv_feat)
...
def train(local_rank, world_size, world_dict, shard_dict, validation = False, Resume = None):
    ...
    if local_rank == 0:
        tsboard_path = "/home/yby/SGD-HFT-Intern/Projects/T0/CNN/tensorboard_logs/cls18_v2"
        if not os.path.exists(tsboard_path):
            os.makedirs(tsboard_path)
        WRITER = SummaryWriter(log_dir=tsboard_path)

    model_path = '/home/yby/SGD-HFT-Intern/Projects/T0/CNN/train_dir_0/model/CNN/param_cls18_v2_adam/'
    ...
    #     optimiser. = model_data['current_step']
    ...
    for epoch_idx in range(prior_epochs, EPOCHS):
        ...
        for batch_idx, (x, y) in enumerate(train_dataloader):
            ...
            h_p18 = net(x.permute(0,2,1).to(local_rank))

            # loss_p2 = lossfn_p2(h_p2, y[..., 0].cuda().long())
            loss_p18 = 1e3 * lossfn_p18(h_p18, y.cuda().long())
        ...
        if local_rank == 0:
            ...
            if validation:
                val_result = validate(net, val_dataset, [lossfn_p18], local_rank)
                total_loss_val, p18_loss_val = val_result['total_loss'], val_result['p18']['loss']
                ...
                p18_recall_val = val_result['p18']['recall']
                p18_precision_val = val_result['p18']['precision']
                # # p2 的auc/roc曲线
                ...
                WRITER.add_scalars(main_tag='AUC/ROC--UP_p18',
                                   ...
                                   global_step=epoch_idx + 1)
                #
                p18_accuracy_val = val_result['p18']['accuracy']
