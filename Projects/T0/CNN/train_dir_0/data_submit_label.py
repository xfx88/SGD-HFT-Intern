...
def judge_cls5(x):
    if x['p_5'] >= 0.0015 and x['rolling5_max'] == 1:
        return 2
    if x['p_5'] <= -0.0015 and x['rolling5_min'] == 1:
        return 1
    return 0
...
def judge_cls18(x):
    if x['p_18'] >= 0.002 and x['rolling18_cntpos'] == 1:
        return 2
    if x['p_18'] <= -0.002 and x['rolling18_cntneg'] == 1:
        return 1
    return 0
...
def parallel_submit_ticker_monthly_numpy_train(db):
    ...
    for month, ticker_dates in ticker_date_dict.items():
        Parallel(n_jobs=48, verbose=10, timeout=10000)(delayed(submit_train_data)(month, ticker, dates, db)
