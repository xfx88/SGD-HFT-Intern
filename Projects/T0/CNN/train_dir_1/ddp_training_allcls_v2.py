...
class ConvLstmNet(nn.Module):
    def __init__(self, in_features, seq_len, out_features):
        ...
        self.conv_1 =  CNNBlock(in_channels= in_features,out_channels = 64,kernel_size=(3, ),dilation=(4,),padding=(4, )) # seq: 64 -> 32

        self.conv_2 = CNNBlock(in_channels=64,out_channels=128,kernel_size=(3,),padding=(1,)) # seq: 32 -> 16

        self.conv_3 = CNNBlock(in_channels=128,out_channels=256,kernel_size=(5,),dilation=(3,),padding=(6,)) # seq: 16 -> 8

        self.conv_4 = CNNBlock(in_channels=256,out_channels=512,kernel_size=(3,),padding=(1,)) # seq: 8 -> 4

        self.downsample1 = nn.Linear(512 * 4, 3)
        self.downsample2 = nn.Linear(512 * 4, 3)
        self.downsample3 = nn.Linear(512 * 4, 3)
...
def train(local_rank, world_size, world_dict, shard_dict, validation = False, Resume = None):
    ...
    if local_rank == 0:
        tsboard_path = "/home/wuzhihan/Projects/CNN/tensorboard_logs/clsallv2"
        if not os.path.exists(tsboard_path):
            os.makedirs(tsboard_path)
        WRITER = SummaryWriter(log_dir=tsboard_path)

    model_path = '/home/wuzhihan/Projects/CNN/train_dir_0/model/CNN/param_clsall_v2_adam/'
    ...
    lossfn_p2 = FocalLoss(alpha=torch.tensor([1/13, 6/13, 6/13]), class_num=3).cuda()
    lossfn_p5 = FocalLoss(alpha=torch.tensor([1/13, 6/13, 6/13]), class_num=3).cuda()
    lossfn_p18 = FocalLoss(alpha=torch.tensor([1/13, 6/13, 6/13]), class_num=3).cuda()
    ...
    for epoch_idx in range(prior_epochs, EPOCHS):
        ...
        for batch_idx, (x, y) in enumerate(train_dataset):
            ...
            loss = 0.3 * loss_p2 + 0.4 * loss_p5 + 0.3 * loss_p18

            total_loss += loss.item()
            loss.backward()
            optimiser.step()

            if (batch_idx + 1) % 30 == 0 or batch_idx == 0:
