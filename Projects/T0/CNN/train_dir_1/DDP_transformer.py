...
def shard_keys(start_date, end_date, seq_len = 50, time_step = 1, db = DB_ID):
    ...
    keys_to_shard = [x.decode(encoding = 'utf-8') for x in all_redis_keys
                    ...
                    and (x.decode(encoding = 'utf-8').split('_')[0] == 'filterlabels'))]
    cnt = 0 # 记录所有序列的长度
    keys_to_shard.sort()
...
def train(local_rank, world_size, world_dict, shard_dict, validation = False, Resume = None):
    ...
    # lossfn_p2 = FocalLoss(alpha = 0.02, gamma = 4).to(local_rank)
    # lossfn_p5 = FocalLoss(alpha = 0.02, gamma = 4).to(local_rank)
    # lossfn_p18 = FocalLoss(alpha = 0.02, gamma = 4).to(local_rank)
    lossfn_p2 = nn.CrossEntropyLoss(weight = torch.tensor([1., 40., 40.])).to(local_rank)
    lossfn_p5 = nn.CrossEntropyLoss(weight = torch.tensor([1., 25., 25.])).to(local_rank)
    lossfn_p18 = nn.CrossEntropyLoss(weight = torch.tensor([1., 20., 20.])).to(local_rank)

    opt = Optimiser(ddp_net, scale_factor=1e-5, warmup_steps=WARMUP)
    if prior_epochs != 0:
