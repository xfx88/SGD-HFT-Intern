...
from lob.modeling.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
# from lib.model import StructMatchNet, FeatureCorrelation, featureL2Norm
#import glog as log
# from lib.cbam import CBAM
import glog as log
import lob._init_paths
# from tools.hrnet import HRNet
from lob.wide_resnet import WideResnet
...
class ConvLstmNet(nn.Module):
    def __init__(self, in_features, seq_len, out_features):
        ...
        self.conv_4 = nn.Sequential(nn.Conv2d(in_channels=256,
                                    ...
                                    nn.BatchNorm2d(num_features=512),
                                    nn.ReLU(inplace=True))
        self.rnn = nn.GRU(512, 64, num_layers=1, batch_first=True)
        self.relu= nn.ReLU(inplace=False)         
        self.layer_5 = nn.Sequential(nn.Linear(in_features=640, out_features=out_features, bias=True))
