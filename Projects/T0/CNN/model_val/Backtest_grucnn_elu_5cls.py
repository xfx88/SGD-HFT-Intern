...
def gen_open_pos(x, times = 1):
    ...
    if (int(x[LABEL_IDX]) == 4) and (int(x[LABEL_IDX + 1]) == 3 or int(x[LABEL_IDX + 1]) == 4):
        return 100 * times
    elif (int(x[LABEL_IDX]) == 2) and (int(x[LABEL_IDX + 1]) == 1 or int(x[LABEL_IDX + 1]) == 2):
        return -100 * times
...
class Predict:
    ...
    def _load_model(self):
        model_path = '/home/yby/SGD-HFT-Intern/Projects/T0/CNN/train_dir_0/model/CNN_param_cls5all_matrix_v2/'
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_2_pred_prob, p_2_pred = label_extractor(p_2_pred, prob_up = 0.7, prob_down = 0.7, cls_num=5)
        p_5_pred_prob, p_5_pred = label_extractor(p_5_pred, prob_up = 0.65, prob_down = 0.65, cls_num=5)
        p_18_pred_prob, p_18_pred = label_extractor(p_18_pred, prob_up = 0.5, prob_down = 0.5, cls_num=5)
        ...
        res['Direction'] = res['predictions'].apply(lambda x: "Buy" if (int(x[LABEL_IDX + 1]) == 4) \
            else \
            ( "Sell" if (int(x[LABEL_IDX + 1]) == 2) else None))
        ...
        buy_close_condition = (res.fillPos > 0) & ((
                                                       res['predictions'].apply(lambda x: int(x[LABEL_IDX]) == 2
                                                                                          or int(x[LABEL_IDX]) == 1
                                                                                          or int(
                                                           x[LABEL_IDX - 1]) == 2)) | res.apply(
            lambda x: abs(x["last"] / x["cost"]) > 1.0025, axis=1))
        sell_close_condition = (res.fillPos < 0) & ((
                                                        res['predictions'].apply(lambda x: int(x[LABEL_IDX]) == 4
                                                                                           or int(x[LABEL_IDX]) == 3
                                                                                           or int(
                                                            x[LABEL_IDX - 1]) == 4)) | res.apply(
            lambda x: abs(x["last"] / x["cost"]) > 1.0025, axis=1))
