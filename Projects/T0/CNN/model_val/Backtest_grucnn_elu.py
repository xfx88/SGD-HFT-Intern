...
class Predict:
    ...
    def _load_model(self):
        ...
        model_name = 'CNNLstmCLS_epoch_9_bs10000_sl64_ts3.pth.tar'
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_5_pred_prob, p_5_pred = label_extractor(p_5_pred, prob_up = 0.8, prob_down = 0.8)
        p_18_pred_prob, p_18_pred = label_extractor(p_18_pred, prob_up = 0.8, prob_down = 0.8)
        ...
        res['Direction'] = res['predictions'].apply(lambda x: "Buy" if (int(x[1]) == 2) else ( "Sell" if (int(x[1]) == 1) else None))
        ...
        buy_close_condition = (res.fillPos > 0) & (
            res['predictions'].apply(lambda x: int(x[LABEL_IDX]) == 1 or int(x[LABEL_IDX]) == 0))
        sell_close_condition = (res.fillPos < 0) & (
            res['predictions'].apply(lambda x: int(x[LABEL_IDX]) == 2 or int(x[LABEL_IDX]) == 0))
        ...
        res.loc[(res['pos'] > 0 | sell_close_condition), 'comm_price'] = res.loc[
            (res['pos'] > 0 | sell_close_condition), 'ask_price1']
        res.loc[(res['pos'] < 0 | buy_close_condition), 'comm_price'] = res.loc[
            (res['pos'] < 0 | buy_close_condition), 'bid_price1']

        res.drop(['predictions'], axis=1, inplace=True)

        res['custom_cls2 | cls2 pred'] = res.apply(lambda x: str(round(x["p_2"], 6)) + " | " + str(x["cls_2"]) + " | " + str(x["cls_2_pred"]), axis = 1)
        res['custom_cls5 | cls5 pred'] = res.apply(lambda x: str(round(x["p_5"], 6))  + " | " + str(x["cls_5"]) + " | " + str(x["cls_5_pred"]), axis = 1)
        res['custom_cls18 | cls18 pred'] = res.apply(lambda x: str(round(x["p_18"], 6)) + " | " + str(x["cls_18"]) + " | " + str(x["cls_18_pred"]), axis = 1)
