...
class Predict:
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_2_pred_prob, p_2_pred = label_extractor(p_2_pred, prob_up=[0.6, 0.6], prob_down=[0.6, 0.6], cls_num=5)
        ...
        buy_close_condition = (res.fillPos > 0) & ((
                                                       res['predictions'].apply(
                                                           lambda x: any([i == 1 or i == 2 for i in x]))) | res.apply(
            lambda x: abs(x["last"] / x["cost"] - 1) >= 0.005, axis=1)
                                                   )
        sell_close_condition = (res.fillPos < 0) & ((
                                                        res['predictions'].apply(
                                                            lambda x: any([i == 3 or i == 4 for i in x]))) | res.apply(
            lambda x: abs(x["last"] / x["cost"] - 1) >= 0.005, axis=1)
                                                    )
