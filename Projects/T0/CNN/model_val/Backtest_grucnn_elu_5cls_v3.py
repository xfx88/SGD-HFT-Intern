...
def gen_open_pos(x, times = 1):
    ...
    #     return -100 * times
    if (int(x[LABEL_IDX]) == 4 or int(x[LABEL_IDX]) == 3):
        return 100 * times
    elif (int(x[LABEL_IDX]) == 2 or int(x[LABEL_IDX]) == 1):
        return -100 * times
...
class Predict:
    ...
    def _load_model(self):
        ...
        model_name = 'CNNLstmCLS_epoch_33_bs12000_sl64_ts3.pth.tar'
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_2_pred_prob, p_2_pred = label_extractor(p_2_pred, prob_up=[0.6, 0.7], prob_down=[0.6, 0.7], cls_num=5)
        p_5_pred_prob, p_5_pred = label_extractor(p_5_pred, prob_up=[0.5, 0.6], prob_down=[0.6, 0.6], cls_num=5)
        p_18_pred_prob, p_18_pred = label_extractor(p_18_pred, prob_up=[0.5, 0.5], prob_down=[0.6, 0.5], cls_num=5)
        ...
        res['Direction'] = res['predictions'].apply(lambda x: "Buy" if ((int(x[LABEL_IDX]) == 4)) \
            else \
            ( "Sell" if ((int(x[LABEL_IDX]) == 2)) else None))
        ...
        buy_close_condition = (res.fillPos > 0) & ((
            ...
            lambda x: abs(x["last"] / x["cost"] - 1) >= 0.005, axis=1))
