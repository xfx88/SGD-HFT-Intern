...
def gen_open_pos(x, times = 1):
    if (int(x[LABEL_IDX]) == 2) and int(x[LABEL_IDX]) == 2:
        return 100 * times
    elif (int(x[LABEL_IDX]) == 1) and (int(x[LABEL_IDX]) == 1):
        return -100 * times
...
class Predict:
    ...
    def _load_model(self):
        ...
        model_name = 'CNNLstmCLS_epoch_49_bs10000_sl64_ts3.pth.tar'
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_2_pred_prob, p_2_pred = label_extractor(p_2_pred, prob_up=[0.8], prob_down=[0.8], cls_num=3)
        p_5_pred_prob, p_5_pred = label_extractor(p_5_pred, prob_up=[0.8], prob_down=[0.8], cls_num=3)
        p_18_pred_prob, p_18_pred = label_extractor(p_18_pred, prob_up=[0.8], prob_down=[0.8], cls_num=3)
        ...
        buy_close_condition = (res.fillPos > 0) & (
                                                       res['predictions'].apply(lambda x: int(x[LABEL_IDX - 1]) == 1
                                                                                          or int(x[LABEL_IDX]) == 1) | res.apply(
            lambda x: abs(x["last"] / x["cost"] - 1) >= 0.002, axis=1))
        sell_close_condition = (res.fillPos < 0) & (
                                                        res['predictions'].apply(lambda x: int(x[LABEL_IDX - 1]) == 2
                                                                                           or int(x[LABEL_IDX]) == 2) | res.apply(
        ...
        res["comm_price"] = res["comm_price"].fillna(method='ffill')

        res.drop(['predictions'], axis = 1, inplace = True)

        # res['custom_cls2/cls2pred'] = res.apply(lambda x: str(x["p_2"]) + str(x["cls_2"]) + " / " + str(x["cls_2_pred"]), axis = 1)
        # res['custom_cls5/cls5pred'] = res.apply(lambda x: str(x["p_5"]) + str(x["cls_5"]) + " / " + str(x["cls_5_pred"]), axis = 1)
        # res['custom_cls18/cls18pred'] = res.apply(lambda x: str(x["p_18"]) + str(x["cls_18"]) + " / " + str(x["cls_18_pred"]), axis = 1)
        #
        # res.rename(columns={"cls_2_prob": "custom_cls2Prob",
        #                     "cls_5_prob": "custom_cls5Prob",
        #                     "cls_18_prob": "custom_cls18Prob"
        #                     }, inplace=True)
...
def specific_stock_batch(stock_ids, start_date, end_date):
    SAVING_PATH = '../prediction/grucnn_elu_v2/clsall_v3_elu_ce_epoch49_cls5_p5bound0.8/'
