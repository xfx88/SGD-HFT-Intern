...
class CNNBlock(nn.Module):
    def __init__(self, in_channels: int,
        ...
        self.conv1 = weight_norm(nn.Conv1d(in_channels=in_channels,
        ...
        self.conv2 = weight_norm(nn.Conv1d(in_channels=_latent_size,
                                           out_channels=out_channels,
                                           kernel_size=(kernel_size,),
                                           padding = (_padding_size,),
                                           dilation = (dilation_size,)))
        self.relu2 = nn.ReLU()
        self.sub_block = nn.Sequential(self.conv1, self.relu1, self.conv2, self.relu2)
...
class ConvLstmNet(nn.Module):
    def __init__(self, in_features, seq_len, out_features):
        ...
        _layers = []
        _levels = [INPUT_SIZE, 64, 128, 192, 96, 48, INPUT_SIZE]
        ...
        self.featureExtractor = weight_norm(nn.Conv1d(in_channels=_levels[-1], out_channels=3, kernel_size=(62,)))


    def forward(self, x):
        x = res = self.bn(x)
        x = self.network(x)
        x = self.featureExtractor(x + res)
...
class Predict:
    ...
    def _load_model(self):
        model_path = '/home/yby/YBY/CNN/train_dir_0/model/CNN/param_clsall_matrix_focal/'
        model_name = 'CNNLstmCLS_epoch_19_bs5000_sl64_ts3.pth.tar'
    ...
    def specific_stock(self, stock_id, date):
        ...
        p_2_pred = label_extractor(p_2_pred, prob_threshold=0.45).reshape((-1, 1))
        p_5_pred = label_extractor(p_5_pred, prob_threshold=0.45).reshape((-1, 1))
        p_18_pred = label_extractor(p_18_pred, prob_threshold=0.55).reshape((-1, 1))
        ...
        pred_y = pd.concat([pred_y, data[['cls_2','cls_5','cls_18']]], axis = 1)
        ...
        res['Direction'] = res['predictions'].apply(lambda x: "Buy" if x[2] == 2 else ( "Sell" if x[2] == 1 else None))
