...
class HFDataset(Dataset):
    ...
    def _load_array(self, array, seq_len, time_step, batch_size):
        ...
        for i in range(1, batch_cnt + 1):
            self._x.append(unfold_array[(i - 1) * batch_size: i * batch_size, :, 1: -4])
...
class HFDatasetCls(Dataset):
    ...
    def _load_array(self, array, seq_len, time_step, batch_size):
        ...
        batch_cnt = math.ceil(unfold_array.size()[0] / batch_size)
        for i in range(1, batch_cnt + 1):
            self._x.append(unfold_array[(i - 1) * batch_size: i * batch_size, :, : -3])
            self._y.append(unfold_array[(i - 1) * batch_size: i * batch_size, -1, -3:].squeeze(1))
        return
