...
class MatrixCrossEntropy(nn.Module):
    def __init__(self, class_weight, label_nums = 3, local_rank = 0, gamma = 2):
        ...
        self.label_weight = torch.Tensor([0.2, 0.4, 0.4]).to(local_rank)
    ...
    def forward(self, pred, target):
        ...
        for i in self.label_nums:
            temp.append(F.one_hot(target[:, i].unsqueeze(1), num_classes = 3))
        temp = torch.cat(temp, dim = 1)
        logsoftmax_pred = F.log_softmax(pred, dim=2)
        softmax_pred = F.softmax(pred, dim=2)
        loss = - ((torch.pow(1 - softmax_pred, self.gamma) * self.class_weight * logsoftmax_pred).sum(1) @ self.label_weight).sum()
...
class CNNBlock(nn.Module):
    def __init__(self, in_channels: int,
        ...
        self.sub_block = nn.Sequential(self.conv1, self.relu1, self.conv2)
...
def train(local_rank, world_size, world_dict, shard_dict, validation = False, Resume = None):
    ...
    if local_rank == 0:
        tsboard_path = "/home/yby/SGD-HFT-Intern/Projects/T0/CNN/tensorboard_logs/clsall_matrix_focal"
        if not os.path.exists(tsboard_path):
            os.makedirs(tsboard_path)
        WRITER = SummaryWriter(log_dir=tsboard_path)

    model_path = '/home/yby/SGD-HFT-Intern/Projects/T0/CNN/train_dir_0/model/CNN/param_clsall_matrix_focal/'
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    if Resume:
        model_name = f'CNNLstmCLS_epoch_10_bs8000_sl64_8blocks_ts3.pth.tar'
    ...
    # optimizer = SGD(ddp_net.parameters(), lr = 1e-4, momentum = 0.2)
...
def main_train(LOGGER):
    ...
    train_start_date = '20210501'
    train_end_date = '20210531'
