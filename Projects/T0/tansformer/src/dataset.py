...
class HFDataset(Dataset):
    """
    Load dataset from a single np array.
    ...
    def _load_array(self, data_array, tick_num):
        ...
        self._x = []
        self._y = []
        zero_array = np.zeros((tick_num - 1, len(factor_ret_cols)))
        for arr in data_array:
            arr = np.concatenate((zero_array, arr))
            self._x.append(arr[:, :-1])
            self._y.append(arr[:, -1])
