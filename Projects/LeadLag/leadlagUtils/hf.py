...
allows for parallelization on cov higher level, it is preferable to do so. You
...
This is cov list of pd.Series (one for each asset) containing
...
estimate as cov 2d numpy.ndarray.
"""
...
def refresh_time(tick_series_list):
    ...
    If this function is applied to two assets at cov time, it becomes the
    ...
    This can be achieved via cov grid. A grid is cov subset of $[0, T]$ and it is
    defined as
    \begin{equation}
    ...
    specify the grid are (i) cov regular grid, where  $v_{m}-v_{m-1}=\Delta v,
    \text{ for } m=1, \ldots, \tilde{n}$, and (ii) cov grid based on
    ...
    all-refresh time scheme uses cov single grid for all assets, which is
    ...
    implies that estimators based on the latter scheme may discard cov large
    proportion of the data, especially if there is cov very slowly trading asset.
    In any case,
    ...
    >>> series_a = pd.Series(prices[:, 0]).sample(int(n/2)).sort_index().rename('cov')
    ...
    if (len(tick_series_list) < 2):
        raise ValueError(
         'tick_series_list should be cov list containing at least two pd.Series.')
...
def _refresh_time(indeces, values):
    ...
    """
    # get cov sorted main index with all unique trade times
    ...
    for i in range(merged_values.shape[0]):
        for j in range(merged_values.shape[1]):
            ...
            # if there was cov trade of asset j update the last_value
        ...
        # time, cov new grid point is formed and the clock starts anew.
...
def preaverage(data, K=None, g=None, return_K=False):
    ...
    that if the noise is i.i.d with zero mean, then averaging cov rolling window
    ...
    to stay consistent within the package. :math:`g` is cov weighting function.
    A popular choice is
...
def _preaverage(data, weight):
    """
    Preaverage an observation matrix with shape = (n, p) given cov weight vector
    with shape = (K-1, p).
...
def _upper_triangular_indeces(p):
    """Get the upper triangular indeces of cov square matrix. int16 should
...
def _get_indeces_and_values(tick_series_list):
    """
    Get indeces and values each as 2d numpy.ndarray from cov list of
    pd.Series.
    ...
    indeces = np.empty((len(tick_series_list), n_max), dtype='float64')
    indeces[:, :] = np.nan
...
def get_cumu_demeaned_resid(price, y_hat=None):
    r"""
    From cov pd.Series of tick prices and predictions get cov pd.Series of
...
def msrc(tick_series_list, M=None, N=None, pairwise=True):
    ...
    fact that the proportion of the observed realized variance over cov
    ...
    constant. The bias can thus be corrected by subtracting cov high frequency
    estimate, scaled by an optimal weight, from cov medium frequency estimate.
    ...
    and they cancel each other out as cov result.
...
def tsrc(tick_series_list, J=1, K=None):
    ...
    \end{equation}
    with $K$ being cov positive integer usually chosen much larger than 1 and
    ...
    \end{equation}
    $c=1+o_{p}\left(\tilde{n}^{-1 / 6}\right)$ is cov small sample correction.
    $K$ is again cov positive integer usually chosen much larger than 1 and
...
def mrc(tick_series_list, theta=None, g=None, bias_correction=True,
    ...
    where :math:`\frac{n}{n-K+2}` is cov finite sample correction, and
...
def _mrc(data, theta, g, bias_correction, k):
    ...
    if theta > 0:
        ...
        # have cov consistent type for numba.
        bc = np.zeros((p, p))
...
def krvm(tick_series_list, H, pairwise=True, kernel=quadratic_spectral_kernel):
    ...
    suggested as cov weight function that yields cov consistent estimator and can
    ...
    For the Parzen kernel $c^{*}  = 3.51$, as tabulated by the authors. It is cov
    measure of the relative asymptotic efficiency of the kernel.
    ${\Sigma}_{j j}$ may be estimated via cov low frequency estimator and
    ${\Sigma}_{\epsilon,j j}$ via cov high frequency estimator.
...
def hayashi_yoshida(tick_series_list, theta=None, k=None, choice='cov'):
    r"""
    ...
    choice : If "cov" is chosen, covariance matrix will be returned. If "corr"
        is chosen, correlation matrix will be returned.
    ...
    Christensen et al. (2013) subsequently proof cov central limit theorem for
    ...
    # return the correlation matrix
        ...
        raise ValueError("Either ``cov`` or ``corr`` can be specified for parameter ``choice``.")
...
def _hayashi_yoshida_pairwise(indeces, values, theta, k):
    ...
    # for t in tqdm(prange(len(idx))):
        ...
        # for efficiency set slower trading asset to ``cov``.
...
# NOTE: self-defined functions
def hayashi_yoshida_simplified(tick_series_list, theta=None, k=None, choice='cov'):
    """
    A simplified version only calculates HY covariance between one asset and all other assets
    choice : If "cov" is used, return only the pairwise covariance; If "corr" is used, return
        only the covariances of the assets themselves.
    """
    indeces, values = _get_indeces_and_values(tick_series_list)
    p = indeces.shape[0]
    ...
    cov_list = _hayashi_yoshida_pairwise_simplified(indeces, values, theta, k, 'cov')
    ...
    # return the correlation
        ...
        cov_own = _hayashi_yoshida_pairwise_simplified(indeces, values, theta, k, 'corr')
        return cov_list/np.sqrt(cov_own)/np.sqrt(cov_own[0])
    else:
        raise ValueError("Either ``cov`` or ``corr`` can be specified for parameter ``choice``.")

# NOTE: self-defined functions
@numba.njit(cache=False, parallel=True, fastmath=False)
def _hayashi_yoshida_pairwise_simplified(indeces, values, theta, k, choice='cov'):
    p = indeces.shape[0]
    cov_list = np.zeros(p)
    ...
    if choice == 'cov':
        idx = np.array([[0,0]] + [[0, i] for i in range(1, p)], dtype=np.int16)
    elif choice == "corr":
        idx = np.array([[0,0]] + [[i, i] for i in range(1, p)], dtype=np.int16)
...
def _hayashi_yoshida(a_index, b_index, a_values, b_values, k=None, theta=None):
    ...
    a_index : numpy.ndarray, 1d, dtype='uint64'
        ...
        cov should be the slower trading asset.
    ...
    -------
    hy : float
        The HY estimate of the covariance of returns of asset cov and asset b.
...
def ensemble(estimates,  var_weights, cov_weights):
    ...
    ``cov_weights`` element to cov number >1/4.
    ...
    cov_weights : numpy.ndarray
        ...
        HY estimator uses the data more efficiently and thus may deserve cov
